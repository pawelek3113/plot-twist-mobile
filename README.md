...
> so you might need to disable that in [`@plot-twist/auth`](./packages/auth/src/index.ts)
...
> In this template, we use `@plot-twist` as a placeholder for package names. As a user, you might want to replace it with your own organization or project name. You can use find-and-replace to change all the instances of `@plot-twist` to something like `@my-company` or `@project-name`.
